...
def call_deepseek_api(markdown_content):
    ...
    6. 输出格式要求：
        - 严格按照以下格式：
            | 序号（对应在原文中的意见序号）| 具体意见（此列要严格列出文档中的具体意见） | 内容简述 | 分类 ｜ 改善建议 |
        - markdown表格中最后一行给出分析总结，分析输入的整体意见内容的数据特点，比如：普通问题占比、尖锐问题占比、高频意见类型等，直接列在最后一行。
    ...
    testing_prompt = """
        ...
        4. 最后将筛选出的尖锐问题以Markdown形式输出，按照原文件中每条意见的顺序列出。
        5. 输出格式要求：
            - 表格中一列严格列出文档中的具体意见，另一列简述意见内容。
            - 每条意见的分类和建议需清晰明了。
            - 表格中最后一行给出分析总结，分析输入的整体意见内容的数据特点，比如：普通问题，尖锐问题占比，高频发意见等。
        6. 仅给出markdown的内容，结果本身就是markdown格式，不用额外的解释说明。
    """
