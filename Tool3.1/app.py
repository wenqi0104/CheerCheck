...
def process_data():
    ...
    if data_source == 'url':
        ...
        if file and allowed_file(file.filename):
            ...
            if '.' in filename and filename.rsplit('.', 1)[1]:
                ...
                # 处理没有扩展名的情况
                return "Invalid file name or missing extension", 400
    ...
    try:
        # 调接口处理md的raw data
        print('------------>等待结果生成中：')
        table_result = backend_methods.call_api(raw_text,manualPrompt)
...
# 整理数据（翻译+分类），实现逻辑是pdf/excel解析为markdown，call api处理结果
...
def organize_data():
    ...
    try:
        # 调接口处理md的raw data
        print('------------>等待结果生成中：')
        table_result = backend_methods.call_api_to_organize(raw_text,manualPrompt)
